...
def homepage(request):
    ...
    return render(request, 'classroom/teachers/home.html', {'teacher': request.user, 'subject':sub, 'student':student} )
...
def profile(request, slug):
    ...
    for x in teacher:
        if slug == x.user.username:
            number = x.user.id
            break
    sub = Teacher.objects.filter(user=number)[0].interests.all()
    t = Teacher.objects.filter(user=number)[0]
    return render(request, 'classroom/teachers/profile.html', {'teacher': t.user, 'subject':sub})
